...
class EcoFriendly(BasePage):
    ...
    def add_product_to_cart(self, selected_product):
        ...
        (ActionChains(self.driver).move_to_element(selected_product).
         ...
         move_to_element(add_to_cart_button).click(add_to_cart_button).perform())
        self.driver.execute_script("window.scrollBy(0, -600);")
        sleep(3)  # по-другому никак не получается. 2 дня мучался.
    ...
    def check_product_name_in_cart(self):
        ...
        assert self.product_name_text == product_name_in_cart_text, "The product names do not match"
